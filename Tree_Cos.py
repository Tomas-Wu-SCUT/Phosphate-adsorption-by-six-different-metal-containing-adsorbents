...
for i in range(0, nTest):
    ...
    for j in range(0, nsj1- nTest):
        B1 = Input.iloc[j, :]
        fz1 = (A1 * B1).sum() 
        fm1 = math.sqrt((A1*A1).sum())
        fm2 = math.sqrt((B1*B1).sum())
        cosSimilary.append(fz1 / (fm1 * fm2))
    
    cosSimilary_stId = sorted(range(len(cosSimilary)), key = lambda k : cosSimilary[k], reverse = True)
    
    x_train = []
    nTrain = 20
    Input_Cos = Input.iloc[cosSimilary_stId[0:nTrain], :]
    Output_Cos = Output.iloc[cosSimilary_stId[0:nTrain]]
