...
for i in range(0, nTest):
    ...
    for j in range(0, nsj1- nTest):
        B1 = Input.iloc[j, :]
        temp = math.sqrt(((A1 - B1)*(A1 - B1)).sum())
        Euclidean.append(temp)
    
    Euclidean_stId = sorted(range(len(Euclidean)), key = lambda k : Euclidean[k], reverse = True)
    
    x_train = []
    nTrain = 20
    Input_Cos = Input.iloc[Euclidean_stId[0:nTrain], :]
    Output_Cos = Output.iloc[Euclidean_stId[0:nTrain]]
